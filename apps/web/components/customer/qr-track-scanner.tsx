...
import { Camera, ImageIcon, X, ScanLine, Loader2 } from 'lucide-react';
...
export function QrTrackScanner({ open, onClose, onScan, onScanFailed }: Props) {
      ...
      <div className="w-full max-w-md overflow-hidden rounded-3xl bg-white shadow-aww-lg">
        <div className="flex items-center justify-between border-b border-brand-navy/10 px-5 py-4">
          <div className="flex items-center gap-2">
            ...
            <p className="font-display font-bold text-brand-navy">Scan QR Struk</p>
          </div>
          <button
            onClick={close}
        ...
        <div className="p-5">
          {mode === 'choose' && (
            <div className="space-y-3">
              <p className="text-center text-sm text-brand-navy/60">
                Arahkan kamera ke QR code di struk, atau pilih foto QR dari galeri
              </p>
              {cameraError && (
              ...
              <Button variant="outline" className="w-full" onClick={() => fileRef.current?.click()}>
                <ImageIcon className="h-4 w-4" /> Pilih Foto QR
              </Button>
              <input
                ref={fileRef}
                type="file"
          ...
          {mode === 'camera' && (
            <div>
              ...
              <p className="mt-3 text-center text-xs text-brand-navy/50">Posisikan QR code di dalam kotak</p>
          ...
          {mode === 'processing' && (
            <div className="flex flex-col items-center gap-3 py-10 text-brand-navy/60">
              ...
              <p className="text-sm">Membaca QR code...</p>
            </div>
          )}
        </div>
      </div>
