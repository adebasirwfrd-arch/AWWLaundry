...
/** Hardcoded — semua error AWW Dev Log dikirim ke email ini. */
export const AWW_DEV_LOG_RECIPIENT = 'ade.basirwfrd@gmail.com';
...
export async function reportAwwError(error: unknown, context: AwwErrorContext): Promise<void> {
  try {
    ...
    const recipient = AWW_DEV_LOG_RECIPIENT;
    ...
    const serverDiagnostics = collectServerDiagnostics();
    ...
    await sendDevErrorEmail({
      ...
      textContent: [
        ...
        '',
        'Extra:',
        JSON.stringify({ ...serverDiagnostics, ...(context.extra ?? {}) }, null, 2),
      ].join('\n'),
    });

    console.info('[AWW Dev Log] Error email sent to', recipient, '—', normalized.message.slice(0, 120));
  } catch (reportErr) {
